...
public class InventoryTestDataFactory {
    ...
    public static Product createProduct(ProductCategory category, int leadTime, int available, String name) {
    ...
    public static Product createExpirableProduct(String name, int leadTime, int available, LocalDate expirationDate) {
        return new Product(null, leadTime, available, ProductCategory.EXPIRABLE, name, expirationDate, null, null);
    }
    ...
    public static List<Product> createTestProducts() {
        ...
        Product watermelon = createProduct(ProductCategory.SEASONAL, 15, 30, "Pumpkin");
        watermelon.setSeasonStartDate(now.minusDays(2));
        watermelon.setSeasonEndDate(now.plusDays(58));

        Product grapes = createProduct(ProductCategory.SEASONAL, 15, 30, "Strawberries");
        grapes.setSeasonStartDate(now.plusDays(180));
        grapes.setSeasonEndDate(now.plusDays(240));

        return List.of(
                createProduct(ProductCategory.NORMAL, 15, 30, "Headphones"),
                createProduct(ProductCategory.NORMAL, 10, 0, "Mouse Pad"),
